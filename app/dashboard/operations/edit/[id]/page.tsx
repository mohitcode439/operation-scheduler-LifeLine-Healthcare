...
export default function EditOperationPage() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        setIsLoading(true)
        logger.info("Fetching operation data and related entities")

        // Fetch operation data
        const operationDocRef = doc(db, "operations", operationId)
        const operationDocSnap = await getDoc(operationDocRef)

        if (!operationDocSnap.exists()) {
          logger.error("Operation not found")
          toast({
            variant: "destructive",
            title: "Error",
            description: "Operation not found. Redirecting to operations list.",
          })
          router.push("/dashboard/operations")
          return
        }

        const operationData = operationDocSnap.data()

        // Convert Firestore timestamp to Date
        let operationDate = new Date()
        if (operationData.date) {
          operationDate = operationData.date.toDate ? operationData.date.toDate() : new Date(operationData.date)
        }
        ...
        form.reset({
          ...
          doctorName: operationData.doctorName || "",
          doctorId: operationData.doctorId || "",
          operationType: operationData.operationType || "",
          operatingTheater: operationData.operatingTheater || "",
          date: operationDate,
          duration: operationData.duration || "",
          status: operationData.status || "scheduled",
          anesthesiaType: operationData.anesthesiaType || "",
          anesthesiologist: operationData.anesthesiologist || "",
          assistantSurgeon: operationData.assistantSurgeon || "",
          nurses: operationData.nurses || "",
          preOpNotes: operationData.preOpNotes || "",
          postOpNotes: operationData.postOpNotes || "",
          requiredMaterials: operationData.requiredMaterials || "",
        })

        logger.info("Operation data and related entities fetched successfully")
      } catch (error) {
        logger.error("Error fetching operation data:", error)
        toast({
          variant: "destructive",
          title: "Error",
          description: "Failed to load operation data. Please try again.",
        })
      } finally {
        setIsLoading(false)
      }
  ...
  }, [user, operationId, db, form, router, toast])
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      ...
      logger.info("Updating operation", { id: operationId })
      ...
      logger.info("Operation updated successfully", { id: operationId })
    ...
    return (
      <DashboardLayout>
        <div className="flex justify-center items-center h-64">
          ...
          <span className="ml-2">Loading operation data...</span>
        </div>
      </DashboardLayout>
    )
  }
      ...
      <div className="flex flex-col space-y-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Update the details of the scheduled operation</CardDescription>
          </CardHeader>
          <CardContent>
            <Form {...form}>
                      ...
                      <FormItem>
                        ...
                        <Select onValueChange={field.onChange} value={field.value}>
                          ...
                          <SelectContent>
                            ...
                            <SelectItem value="Cholecystectomy">Cholecystectomy (Gallbladder Removal)</SelectItem>
                            <SelectItem value="Coronary Bypass">Coronary Bypass</SelectItem>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input placeholder="Nurse names separated by commas" {...field} />
                        </FormControl>
